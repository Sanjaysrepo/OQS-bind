...
/* an in-flight fragment request, see dns_dispatch_send_fragment() */
typedef struct {
...
udp_recv(isc_nmhandle_t *handle, isc_result_t eresult, isc_region_t *region,
	...
	bool respond = true;
	isc_time_t now;
	uint8_t udp_fragmentation_mode;
	unsigned frag_nr = 0, nr_fragments = 0;
	isc_buffer_t *reassembled = NULL;
	...
	/*
	 * UDP fragmentation (see dns/qbf.h and dns/raw.h).  Fragments are
	 * collected in the fragment cache and the remaining ones are
	 * requested; the caller only gets to see the reassembled reply.
	 */
	udp_fragmentation_mode = disp->mgr->udp_fragmentation_mode;
	if (udp_fragmentation_mode == 1 && (flags & DNS_MESSAGEFLAG_TC) != 0) {
		/*
		 * QBF: every fragment is a parseable message marked with TC
		 * and EDNS option 22; the first one is not requested.
		 */
		...
		dns_message_create(disp->mgr->mctx, DNS_MESSAGE_INTENTPARSE,
				   &msg);
		result = dns_message_parse(msg, &buf,
		    ...
		    is_fragment_opt(msg) == ISC_R_SUCCESS)
		{
			...
			unsigned qbf_nr = msg->nr_fragments;

			fcache_create_key(id, to_addr_buf, key, &keysize);
			dispentry_log(resp, LVL(90),
				      "QBF fragment %lu of %u for %s",
				      msg->fragment_nr, qbf_nr, key);

			if (msg->fragment_nr > 0) {
				result = fcache_add_fragment(fcache, key,
							     keysize, msg);
				if (result == ISC_R_SUCCESS) {
					dns_message_t *out_msg = NULL;
					result = reassemble_fragments(
						disp->mgr->mctx, fcache, key,
						keysize, &out_msg);
					if (result == ISC_R_SUCCESS) {
						dispentry_log(
							resp, LVL(90),
						...
						goto done;
					} else if (result != ISC_R_INPROGRESS) {
						dispentry_log(
							resp, LVL(90),
							"QBF reassembly "
							"failed: %s",
				...
				result = fcache_add_with_fragment(
					fcache, key, keysize, msg, qbf_nr);
				if (result == ISC_R_SUCCESS) {
					/* ?N?qname requests are 1-based */
					for (unsigned i = 2; i <= qbf_nr;
					     i++)
					{
						isc_buffer_t *qbuf = NULL;
						result = create_fragment_query_qname(
							disp->mgr->mctx, &buf,
							i, &qbuf);
						if (result != ISC_R_SUCCESS) {
							dispentry_log(
						...
						dns_dispatch_send_fragment(
							resp, &qbuf);
					}
				} else {
					dispentry_log(resp, LVL(90),
						      "could not cache first "
						      "QBF fragment: %s",
						      isc_result_totext(result));
				}
			}
			dns_message_detach(&msg);
			/* keep waiting for the remaining fragments */
			goto next;
		}
		dns_message_detach(&msg);
	} else if (udp_fragmentation_mode == 2 &&
		   raw_parse_envelope(region, &frag_nr, &nr_fragments, NULL) ==
			   ISC_R_SUCCESS)
	{
		/*
		 * RAW: the datagram is an envelope (RCODE 12, option 22)
		 * followed by a slice of the real reply.
		 */
		fcache_t *fcache = disp->mgr->fcache;
		char to_addr_buf[ISC_SOCKADDR_FORMATSIZE];
		unsigned char key[69];
		...
		dispentry_log(resp, LVL(90), "RAW fragment %u of %u for %s",
			      frag_nr, nr_fragments, key);

		if (frag_nr == 0) {
			result = fcache_add(fcache, key, keysize, nr_fragments);
			first = (result == ISC_R_SUCCESS);
		}
		result = fcache_add_fragment_buffer(fcache, key, keysize,
						    frag_nr, &frag_buf);
		if (result != ISC_R_SUCCESS) {
			dispentry_log(resp, LVL(90),
				      "could not cache RAW fragment %u: %s",
				      frag_nr, isc_result_totext(result));
			goto next;
		}
		if (first) {
		...
		if (result == ISC_R_SUCCESS) {
			...
			if (result == ISC_R_SUCCESS) {
				region->base = isc_buffer_base(reassembled);
				region->length =
					isc_buffer_usedlength(reassembled);
				dispentry_log(resp, LVL(90),
...
dns_dispatch_initfcache(dns_dispatchmgr_t *mgr, isc_loopmgr_t *loopmgr) {
	...
	REQUIRE(mgr->fcache == NULL);
	// fragments stay for 10 seconds, expiry runs every 30 seconds
	// TODO: turn into settings
	fcache_init(&mgr->fcache, loopmgr, 10, 30);
}

void
dns_dispatch_deinitfcache(dns_dispatchmgr_t *mgr) {
	REQUIRE(VALID_DISPATCHMGR(mgr));
	/*
	 * Must run on the loop that called dns_dispatch_initfcache() (the
	 * expiry timer lives there); named does this from shutdown_server().
	 */
	if (mgr->fcache != NULL) {
...
send_done_fragment(isc_nmhandle_t *handle, isc_result_t result, void *cbarg) {
	...
	dispentry_log(resp, LVL(90), "sent fragment request: %s",
		      isc_result_totext(result));
	if (result != ISC_R_SUCCESS) {
	...
	isc_mem_put(disp->mgr->mctx, ctx, sizeof(*ctx));
...
void
dns_dispatch_send_fragment(dns_dispentry_t *resp, isc_buffer_t **bufp) {
	REQUIRE(VALID_RESPONSE(resp));
	REQUIRE(VALID_DISPATCH(resp->disp));
	REQUIRE(bufp != NULL && *bufp != NULL);

	dns_dispatch_t *disp = resp->disp;
	isc_nmhandle_t *sendhandle = NULL;
	isc_region_t r;
	fragment_send_ctx_t *ctx = isc_mem_get(disp->mgr->mctx, sizeof(*ctx));

	*ctx = (fragment_send_ctx_t){ .buffer = *bufp };
	*bufp = NULL;
	dns_dispentry_attach(resp, &ctx->resp); /* DISPENTRY007 */

	dispentry_log(resp, LVL(90), "sending fragment request");
	switch (disp->socktype) {
	case isc_socktype_udp:
	...
	isc_nm_send(sendhandle, &r, send_done_fragment, ctx);
}
