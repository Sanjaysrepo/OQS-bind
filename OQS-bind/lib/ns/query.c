...
ns__query_start(query_ctx_t *qctx) {
	...
	/*
	 * QBF UDP fragmentation: a ?N?qname query asks for a cached
	 * fragment (RAW uses OPCODE 7, handled in ns_client_request()).
	 */
	if (qctx->client->manager->sctx->udp_fragmentation_mode == 1 &&
	    is_fragment_qname(qctx->client->manager->mctx,
			      qctx->client->message))
	{
		ns_client_log(qctx->client, NS_LOGCATEGORY_CLIENT,
			      NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG(3),
			      "received a QBF fragment query");
