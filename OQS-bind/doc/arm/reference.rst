...
.. namedconf:statement:: udp-fragmentation
   :tags: query, server
   :short: Splits oversized UDP replies into fragments that the resolver requests and reassembles.

   Post-quantum DNSSEC keys and signatures make many replies far larger
   than the 1232-byte UDP limit, so a standard resolver would retry
   almost every signed answer over TCP.  With :any:`udp-fragmentation`
   enabled, an authoritative server splits such a reply into several UDP
   datagrams (fragments) and a resolver asks for the remaining fragments
   and reassembles the reply itself, with no TCP round trip.  The option
   must be set to the same value on both the authoritative server and
   the resolver; it is an OQS-Bind extension and is not understood by
   other DNS software.  The default is to not fragment.

   ``RAW``
      The complete rendered reply is treated as a byte stream and cut
      into slices.  Every fragment is a small DNS message with the
      original question, RCODE 12 and EDNS option 22 carrying the
      fragment number and total (up to 63 fragments), followed by the
      slice.  The resolver requests fragment *n* with a query of OPCODE 7
      carrying the same option, and concatenating the slices yields the
      original reply byte for byte.  RAW works for any record type and
      size (including SPHINCS+ signatures and Falcon-1024 keys) and
      needs the client to use EDNS; clients without EDNS receive the
      usual truncated reply and retry over TCP.  Resolvers that do not
   ...
   ``QBF``
      ...
      the DNSKEY and RRSIG RDATA is split proportionally; fragments are
      marked with the TC flag and EDNS option 22, and the resolver asks
      for fragment *N* by querying ``?N?qname``.  Only DNSKEY and RRSIG
      data can be split, so replies whose size comes from other record
      types still fall back to TCP.

   Fragments are kept in a server-side cache for 30 seconds and in the
   resolver's cache for 10 seconds while the remaining fragments are
   collected.  :iscman:`dig` understands RAW fragments and shows the
   reassembled reply.  See ``lib/dns/include/dns/raw.h`` in the source
   distribution for the exact wire format.
