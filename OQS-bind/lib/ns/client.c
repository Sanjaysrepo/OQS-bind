...
ns_client_send(ns_client_t *client) {
	...
	/* keep RR order stable: QBF fragmentation depends on it */
	render_opts |= DNS_MESSAGERENDER_ORDERED;
	...
	/*
	 * UDP fragmentation (see dns/qbf.h and dns/raw.h).
	 *
	 * A fragment request is answered straight from the fragment cache.
	 * Any other reply is handed to the fragmenter first; if it does not
	 * fit in one datagram the fragments are cached and fragment 0 is
	 * sent instead of the normal rendering.
	 */
	udp_fragmentation_mode = client->manager->sctx->udp_fragmentation_mode;
	if (udp_fragmentation_mode != 0 && !TCP_CLIENT(client)) {
		...
		isc_sockaddr_format(&client->peeraddr, addr_buf,
				    sizeof(addr_buf));
		fcache_create_key(client->message->id, addr_buf, key,
				  &keysize);

		if (udp_fragmentation_mode == 1) {
			/* QBF: ?N?qname queries ask for fragment N-1 */
			is_request = client->message->is_fragment;
		} else {
			/* RAW: OPCODE 7 queries carry EDNS option 22 */
			is_request = (client->message->opcode ==
				      dns_opcode_fragment);
		}
		...
		if (is_request) {
			...
			if (result == ISC_R_SUCCESS) {
				ns_client_log(client, NS_LOGCATEGORY_CLIENT,
					      NS_LOGMODULE_CLIENT,
					      ISC_LOG_DEBUG(3),
					      "sending cached fragment %lu "
					      "(%u bytes) to %s",
					      client->message->fragment_nr,
					      isc_buffer_usedlength(out_frag),
					      addr_buf);
				goto sendfragment;
			}
			ns_client_log(client, NS_LOGCATEGORY_CLIENT,
				      NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG(3),
				      "fragment %lu for %s not in cache: %s; "
				      "answering FORMERR",
				      client->message->fragment_nr, addr_buf,
				      isc_result_totext(result));
			client->message->rcode = dns_rcode_formerr;
			client->message->flags |= DNS_MESSAGEFLAG_QR;
			if (udp_fragmentation_mode == 1) {
				/* QBF marks all fragment replies with TC */
		...
		} else if (udp_fragmentation_mode == 1) {
			/*
			 * hardcoded 1232 since client->udpsize can be set to
			 * 512 as well..
			 */
			client->message->opt = client->opt;
			result = fragment(client->manager->mctx, fcache,
					  client->message, addr_buf, 1232);
			client->message->opt = NULL;
			if (result == ISC_R_SUCCESS &&
			    fcache_get_fragment(fcache, key, keysize, 0,
			...
			if (result != ISC_R_SUCCESS && result != ISC_R_RANGE) {
				ns_client_log(client, NS_LOGCATEGORY_CLIENT,
					      NS_LOGMODULE_CLIENT,
					      ISC_LOG_DEBUG(3),
					      "QBF fragmentation failed: %s",
					      isc_result_totext(result));
			}
		} else if (client->opt != NULL) {
			unsigned nr_fragments = 0;
			result = raw_fragment(client->manager->mctx, fcache,
					      client->message, client->opt,
					      addr_buf,
					      RAW_DEFAULT_MAX_UDP_SIZE,
					      render_opts | preferred_glue,
					      &nr_fragments);
			if (result == ISC_R_SUCCESS &&
			    fcache_get_fragment(fcache, key, keysize, 0,
						&out_frag) == ISC_R_SUCCESS)
			{
				ns_client_log(client, NS_LOGCATEGORY_CLIENT,
					      NS_LOGMODULE_CLIENT,
					      ISC_LOG_DEBUG(3),
					      "RAW: reply for %s split into %u "
					      "fragments, sending fragment 0",
					      addr_buf, nr_fragments);
				goto sendfragment;
			}
			if (result != ISC_R_SUCCESS && result != ISC_R_RANGE) {
				ns_client_log(client, NS_LOGCATEGORY_CLIENT,
	...
	sendfragment:
		/*
		 * Copy the cached datagram into the client's send buffer so
		 * the cache entry may expire while the send is in flight.
		 */
		INSIST(isc_buffer_usedlength(out_frag) <=
		       NS_CLIENT_SEND_BUFFER_SIZE);
		isc_buffer_init(&buffer, client->sendbuf,
				NS_CLIENT_SEND_BUFFER_SIZE);
		isc_buffer_putmem(&buffer, isc_buffer_base(out_frag),
				  isc_buffer_usedlength(out_frag));
		goto sendbuffer;
	}
...
ns_client_request(isc_nmhandle_t *handle, isc_result_t eresult,
	...
	case dns_opcode_fragment:
		CTRACE("fragment");
		if (client->manager->sctx->udp_fragmentation_mode != 2) {
			ns_client_error(client, DNS_R_NOTIMP);
			break;
		}
		/* RAW fragment request: answered from the fragment cache */
		(void)is_fragment_opt(client->message);
		ns_client_send(client);
		break;
	default:
