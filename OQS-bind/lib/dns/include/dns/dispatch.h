...
/*%<
 * Create the resolver-side fragment cache (UDP fragmentation).
 */

void
dns_dispatch_deinitfcache(dns_dispatchmgr_t *mgr);
/*%<
 * Destroy the fragment cache; call on the loop that created it, before
 * the last reference to the manager goes away.
 */
...
/*%<
 * Send a fragment request (UDP fragmentation) on the socket of 'resp'.
 * Ownership of the dynamic buffer '*bufp' passes to the dispatch; it is
 * freed when the send completes.
 */
/*%<
