...
/* an in-flight RAW fragment request */
...
raw_fragment_send_done(isc_nmhandle_t *handle, isc_result_t result, void *arg) {
	...
	debug("RAW: fragment request sent: %s", isc_result_totext(result));
...
recv_done(isc_nmhandle_t *handle, isc_result_t eresult, isc_region_t *region,
	...
	if (!l->tcp_mode && raw_is_fragment(region)) {
		...
		if (rres != ISC_R_SUCCESS) {
			dighost_warning("could not cache RAW fragment %u: %s",
					frag_nr, isc_result_totext(rres));
			goto cancel_lookup;
		}
		...
		if (rres == ISC_R_INPROGRESS) {
			...
			*ctx = (raw_fragment_send_ctx_t){ .buffer = request };
			...
			rres = raw_reassemble(mctx, entry, &full);
			fcache_free_taken_entry(dig_fcache, entry);
			if (rres != ISC_R_SUCCESS) {
				dighost_warning("RAW reassembly failed: %s",
						isc_result_totext(rres));
				goto cancel_lookup;
			}
			debug("RAW: reassembled %u bytes from %u fragments",
			      isc_buffer_usedlength(full), nr_fragments);
			dns_message_reset(msg, DNS_MESSAGE_INTENTPARSE);
			isc_buffer_init(&b, isc_buffer_base(full),
					isc_buffer_usedlength(full));
			isc_buffer_add(&b, isc_buffer_usedlength(full));
			result = dns_message_parse(msg, &b, parseflags);
			dns_message_takebuffer(msg, &full);
		} else {
			dighost_warning("RAW fragment cache lookup failed: %s",
					isc_result_totext(rres));
			goto cancel_lookup;
		}
	}
...
run_loop(void *arg) {
	UNUSED(arg);
	/* no expiry timer: it would keep the event loop alive after the lookup */
	fcache_init(&dig_fcache, NULL, 10, 30);
	start_lookup();
}
