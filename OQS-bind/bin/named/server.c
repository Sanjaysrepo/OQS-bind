...
load_configuration(const char *filename, named_server_t *server,
	...
	if (result == ISC_R_SUCCESS) {
		if (strcmp(cfg_obj_asstring(obj), "QBF") == 0) {
			...
			perror("Fragmentation mode not implemented!\n");
			exit(0);
		}
	}
...
shutdown_server(void *arg) {
	...
	/*
	 * The UDP fragmentation caches own timers on this loop; destroy
	 * them here rather than from whichever thread drops the last
	 * reference to their owners.
	 */
	dns_dispatch_deinitfcache(named_g_dispatchmgr);
	ns_server_deinitfcache(server->sctx);
